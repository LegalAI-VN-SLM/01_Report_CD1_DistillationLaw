...
Each of the three phases is configured with a conservative hyperparameter setting tailored to its objective. Both refinement tracks---Phase 2 (on-policy) and the Phase 1$\rightarrow$Phase 3 offline track---start from the same supervised fine-tuned (SFT) base. The configurations are summarized in Tables~\ref{tab:hparams_phase1}, \ref{tab:hparams_phase2}, and \ref{tab:hparams_phase3}.

\begin{table}[htbp]
\centering
\caption{Hyperparameter configurations across the three training phases.}
\label{tab:hparams}

\begin{subtable}{\linewidth}
\centering
\caption{Phase 1 (Offline Logit Distillation)}
\label{tab:hparams_phase1}
\begin{tabular}{lc}
\hline
\textbf{Hyperparameter} & \textbf{Value} \\ \hline
Learning rate $\eta$    & $2.0 \times 10^{-5}$ \\
Distillation weight $\alpha$ & $0.3$ ($70\%$ CE anchor, $30\%$ KD loss) \\
Temperature $T$         & $1.5$ \\
Top-$K$ logits cache size & $50$ \\
...
\begin{subtable}{\linewidth}
\centering
\caption{Phase 2 (On-Policy Knowledge Distillation)}
\label{tab:hparams_phase2}
\begin{tabular}{lc}
\hline
\textbf{Hyperparameter} & \textbf{Value} \\ \hline
Learning rate $\eta$    & $2.0 \times 10^{-5}$ \\
Distillation weight $\alpha$ & $0.5$ ($50\%$ SFT anchor, $50\%$ on-policy KD) \\
Sampling temperature $T$ & $0.7$ \\
Nucleus sampling top-$p$ & $0.9$ \\
KL clipping threshold $C$ & $5.0$ \\
Effective batch size    & $32$ \\ \hline
\end{tabular}
\end{subtable}
...
\begin{subtable}{\linewidth}
\centering
\caption{Phase 3 (Diagnosis-Driven DPO Alignment)}
\label{tab:hparams_phase3}
\begin{tabular}{lc}
\hline
\textbf{Hyperparameter} & \textbf{Value} \\ \hline
Learning rate $\eta$    & $5.0 \times 10^{-6}$ \\
DPO loss weight $\beta$ & $0.1$ \\
Optimizer               & \texttt{adamw\_8bit} \\
Max context (prompt / response) & $2{,}048$ ($1{,}024$) \\
Epochs                  & $2$ \\ \hline
\end{tabular}
\end{subtable}
\end{table}
