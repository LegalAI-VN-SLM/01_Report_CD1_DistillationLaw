...
- 2.1.2 Legal Reasoning Challenges *(Khó khăn của SLMs: nhảy cóc logic, thiên kiến từ vựng, hiểu sai điều luật)*
...
- 3.2.1 Supervised Training with Anchor *(Bảo toàn tri thức cũ bằng CE loss trên nhãn cứng)*
- 3.2.2 Soft-label training objective with Top-K Logits *(KL Divergence loss trên top-50 logits của Teacher)*
- 3.2.3 Regularization via Quantized Low-Rank Adaptation *(Đóng băng base weights 4-bit chống catastrophic forgetting)*
- 3.2.4 Hyperparameter Corrections *(LR 1e-4→2e-5, alpha 0.5→0.3, batch 16→64 — fix catastrophic forgetting)*
...
- 3.3.4 Kullback-Leibler Divergence Clipping *(Ngăn XML structural tokens (`<think>`, `<major_premise>`) chiếm hết gradient — clip tại ngưỡng C)*
...
- 3.4.3 Rule-Based vs. Large Language Model Judge *(So sánh 2 phương pháp tạo preference pairs)*
...
- 4.1.3 Direct Preference Optimization Preference Data *(Rule-based: 2313/2283 pairs; LLM Judge: 2065/1434 pairs)*
...
- 4.3.1 Impact of Hyperparameter Tuning *(Chứng minh LR quá cao gây catastrophic forgetting)*
- 4.3.2 Low-Rank Adaptation vs. Quantized Low-Rank Adaptation *(QLoRA thắng nhờ implicit regularization)*
...
- 4.3.5 Offline vs. On-Policy Knowledge Distillation *(So sánh chính — Main Contribution)*
- 4.3.6 Direct Preference Optimization Alignment Impact *(KD → KD+DPO cải thiện về logic suy luận)*

**4.4 Ablation Study**
- 4.4.1 Quantized Low-Rank Adaptation vs Low-Rank Adaptation *(CE loss trend chứng minh QLoRA ngăn forgetting)*
- 4.4.2 Rule-Based vs. Large Language Model Judge *(LLM Judge chính xác hơn: A3 OK 44.9% vs 12.3%)*
